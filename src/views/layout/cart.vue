    ...
    <div class="cart">
      ...
      <div class="footer-fixed">
        ...
        <div class="all-total">
          <div class="price">
            <span>合计：</span>
            <span>¥ <i class="totalPrice">{{ selTotal }}</i></span>
          </div>
          <div v-if="!isedit"  class="goPay" :class="{ disabled: selNum === 0 }" @click="goPay" >结算({{ selNum }})</div>
          <div v-else :class="{ disabled: selNum === 0 }" class="delete" @click="handledel">删除</div>
...
export default {
  ...
  computed: {
    ...
    ...mapGetters('cart', ['cartTotal', 'selCartList', 'selNum', 'selTotal', 'isCheckedAll']),
    isLogin () {
  ...
  methods: {
    ...
    toggleAllChecked () {
      this.$store.commit('cart/toggleAllChecked', !this.isCheckedAll)
    },
